...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'DevSecretKeyForReservasApp')
...
# ==============================================================================
# 3. MODELOS E ESTRUTURAS DE DADOS
# ==============================================================================
...
# --- 4.1. Conexão e Helpers Genéricos ---
...
def get_global_setting(key, default_value=None):
    """Busca uma configuração global da tabela 'configuracoes_globais'."""
...
def set_global_setting(key, value):
    """Salva uma configuração global na tabela 'configuracoes_globais'."""
    conn = get_db_connection()
    try:
        conn.execute(
...
def get_regras_reservas():
    """Busca as regras de antecedência de reservas."""
    conn = get_db_connection()
    regras = conn.execute(
        "SELECT antecedencia_minima_dias, antecedencia_maxima_dias FROM regras_reservas WHERE id = 1").fetchone()
    conn.close()
    if regras:
        return {'min_dias': regras['antecedencia_minima_dias'], 'max_dias': regras['antecedencia_maxima_dias']}
    return {'min_dias': 1, 'max_dias': 365}


def set_regras_reservas(min_dias, max_dias):
    """Salva as regras de antecedência de reservas."""
    conn = get_db_connection()
    try:
        conn.execute(
            "INSERT OR REPLACE INTO regras_reservas (id, antecedencia_minima_dias, antecedencia_maxima_dias) VALUES (1, ?, ?)", (min_dias, max_dias))
        conn.commit()
        return True
    except sqlite3.Error as e:
        app.logger.error(
...
# --- 4.2. Setup e Inicialização do DB ---


def init_db():
    """Inicializa o banco de dados, criando tabelas e aplicando migrações simples."""
    conn = get_db_connection()
    cursor = conn.cursor()
    app.logger.info(
    ...
    tabelas = {
        "usuarios": '''CREATE TABLE IF NOT EXISTS usuarios (
            id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT, email TEXT UNIQUE NOT NULL, senha TEXT NOT NULL,
            is_admin INTEGER DEFAULT 0 CHECK(is_admin IN (0, 1)), tipo_usuario TEXT DEFAULT 'cliente' NOT NULL,
            telefone TEXT, foto_perfil TEXT
        )''',
        "empreendimentos": '''CREATE TABLE IF NOT EXISTS empreendimentos (
            id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT UNIQUE NOT NULL, ativo INTEGER DEFAULT 1 CHECK(ativo IN (0, 1))
        )''',
        "tipos_agendamento": '''CREATE TABLE IF NOT EXISTS tipos_agendamento (
            id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT UNIQUE NOT NULL, ativo INTEGER DEFAULT 1 CHECK(ativo IN (0, 1)),
            duracao_minutos INTEGER DEFAULT 60 NOT NULL
        )''',
        "configuracoes_globais": '''CREATE TABLE IF NOT EXISTS configuracoes_globais (chave TEXT PRIMARY KEY, valor TEXT)''',
        "regras_reservas": '''CREATE TABLE IF NOT EXISTS regras_reservas (
            id INTEGER PRIMARY KEY AUTOINCREMENT, antecedencia_minima_dias INTEGER NOT NULL DEFAULT 1,
            antecedencia_maxima_dias INTEGER NOT NULL DEFAULT 365
        )''',
        "unidades": '''CREATE TABLE IF NOT EXISTS unidades (
            id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT NOT NULL, empreendimento_id INTEGER NOT NULL,
            ativo INTEGER DEFAULT 1 CHECK(ativo IN (0, 1)),
            FOREIGN KEY (empreendimento_id) REFERENCES empreendimentos(id) ON DELETE CASCADE, UNIQUE(nome, empreendimento_id)
        )''',
        "horarios_funcionamento": '''CREATE TABLE IF NOT EXISTS horarios_funcionamento (
            id INTEGER PRIMARY KEY AUTOINCREMENT, empreendimento_id INTEGER NOT NULL, dia_semana INTEGER NOT NULL,
        ...
        "agente_tipos_servico": '''CREATE TABLE IF NOT EXISTS agente_tipos_servico (
            agente_id INTEGER NOT NULL, tipo_id INTEGER NOT NULL, PRIMARY KEY (agente_id, tipo_id),
        ...
        "agendamentos": '''CREATE TABLE IF NOT EXISTS agendamentos (
            id INTEGER PRIMARY KEY AUTOINCREMENT, usuario_id INTEGER, tipo_id INTEGER NOT NULL, unidade_id INTEGER NOT NULL,
            data TEXT NOT NULL, hora TEXT NOT NULL, observacoes TEXT, contato_agendamento TEXT,
            status TEXT DEFAULT 'Pendente' NOT NULL, agente_atribuido_id INTEGER,
    ...
    for nome, schema in tabelas.items():
        ...
        app.logger.info(f"Tabela '{nome}' verificada/criada.")
...
def criar_usuario_inicial():
    """Garante que o usuário administrador principal exista."""
    conn = get_db_connection()
    ...
    try:
        ...
        if cursor.fetchone() is None:
            cursor.execute('INSERT INTO usuarios (nome, email, senha, is_admin, tipo_usuario) VALUES (?, ?, ?, ?, ?)',
                           (admin_nome, admin_email, senha_hash, 1, 'admin'))
            app.logger.info(
                f"Usuário administrador inicial '{admin_email}' criado.")
        conn.commit()
    except sqlite3.Error as e:
        app.logger.error(
...
# --- 5.1. User Loader e Funções de Autenticação ---

@login_manager.user_loader
def load_user(user_id):
    """Carrega um usuário pelo ID para a sessão do Flask-Login."""
    conn = get_db_connection()
    row = conn.execute("SELECT * FROM usuarios WHERE id = ?",
                       (user_id,)).fetchone()
    conn.close()
    if row:
        return Usuario(id=row["id"], nome=row["nome"], email=row["email"], senha_hash=row["senha"], is_admin=row["is_admin"],
                       tipo_usuario=row["tipo_usuario"], telefone=row["telefone"], foto_perfil=row["foto_perfil"])
    return None
...
def agente_required(f):
    ...
    @login_required
    def decorated_function(*args, **kwargs):
        if not (current_user.is_agente or current_user.is_admin_user):
...
# ==============================================================================
# 6. FUNÇÕES AUXILIARES DE LÓGICA (HELPERS)
# ==============================================================================

# --- 6.1. Helpers de Agendamento e Slots ---
...
# --- 7.1. Rotas Públicas e de Autenticação ---
...
# --- 7.2. Rotas de Perfil de Usuário ---
...
def painel_agente():
    ...
    try:
        ...
        flash(
            f'Erro ao carregar agendamentos do agente: {str(e)}', 'error')
        app.logger.error(
            f'Erro DB em painel_agente: {str(e)}', exc_info=True)
    finally:
        conn.close()
...
# --- 7.5. Rotas de Administração (Painel /configuracoes) ---
...
# Gerenciamento de Tipos de Agendamento
...
# Gerenciamento de Usuários (Agentes e Admins)
...
# Gerenciamento de Horários e Regras
...
def debug_user():
    return jsonify({
        ...
        'is_authenticated': current_user.is_authenticated, 'is_admin': current_user.is_admin,
        'tipo_usuario': current_user.tipo_usuario, 'telefone': current_user.telefone
    })
